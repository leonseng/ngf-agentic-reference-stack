...
- A Kubernetes cluster with NGF v2.3.0+ installed with inference extension support
...
**What just happened?**
NGF now routes two distinct services from a single Gateway: the frontend on one hostname and the OpenAI-compatible backend API on another. This is the API gateway pattern — a single data-plane entry point with host-based routing to multiple upstream services.
...
This applies the ServiceAccount, RBAC resources, ConfigMap, Deployment, and Service for the EPP in one command. The EPP exposes a gRPC endpoint on port 9002 which NGF calls to resolve the best backend pod for each inference request.
...
Send a message. The full request path is:

```
Browser → NGF:8080 → frontend pod
                        │ (API call)
                        ▼
                  NGF:8080 → backend pod
                                  │ (chat completion POST /v1/chat/completions)
                                  ▼
                          NGF:8000 → EPP (port 9002)
                                          │ (selected replica)
                                          ▼
                                   vLLM simulator
```
